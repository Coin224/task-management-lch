...
(function ($) {
	...
	function show(target) {
		...
		var myTask = {};
		$.ajax({
			url:"staff/showTaskByDate",
			type:"post",
			async:false,//默认是true 异步请求
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				...
				var m_month = day[1];
				var m_day = day[2];
				for(var k = 0; k < myTask.length; k++) {
					var task = myTask[k];
					//alert(info_create_date+"======"+m_year+"-"+m_month+"-"+ m_day);
					//判断任务日期
					//如果日期和任务日期相等，那么展示
					if(task.info_create_date == m_year+"-"+m_month+"-"+ m_day) {
						//展示任务
						taskCount++;
						var taskInfo = task.info_name;


						//todo 根据任务等级 显示不同的颜色
						var taskLevel = task.info_level;
						var color;
						if (taskLevel == 1) {
							color = "black";
						}else if (taskLevel == 2) {
						...
						if(taskInfo.length > 10) {
							taskInfo = taskInfo.substring(0,10)+"...";
						}
						dayHtml += "<br><div align='left' ><font color='"+color+"'>"+taskCount+": "+taskInfo+"</font></div>";//这里面全是字符串，别忘了拼接
					}
				}
				...
				day.dblclick(function (e) {
					...
					var inf = $(this).data('info');
					//打开dialog并且设置title
					$('#taskDlg').dialog('open').dialog("setTitle","<span style='color:blue;font-family: 微软雅黑;'>填写任务</span>");
					//获取创建任务的时间
					$('#info_create_date').val(inf.sYear+"-"+inf.sMonth+"-"+inf.sDay);
				});
			}
		}
